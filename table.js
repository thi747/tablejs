 ...
 * @version 0.984
 *
 * @history 0.984 2018-07-05 Added Sort.size and size_comma to sort Byte's family
 * @history 0.983 2018-06-20 Change filter regex to ignore case and search anywhere on string
 * @history 0.982 2018-06-15 More additional date parsing formats
...
var Sort = (function(){
	...
	// This conversion is generalized to work for either a decimal separator of , or .
	sort.size_converter = function(separator) {
		return function(val) {
			if (typeof(val)=="string") {
				var mul = units.indexOf(val.toUpperCase().split(/.*\d ?/).pop());
				val = parseFloat(val.replace(/^[^\d\.]*([\d., ]+).*/g,"$1").replace(new RegExp("[^\\\d"+separator+"]","g"),'').replace(/,/,'.')) || 0;
				val *= Math.pow(1024,mul);
			}
			return val || 0;
		};
	};
	...
	sort.size = function(a,b) {
		return (sort.size.convert(a)-size.size.convert(b));
	};
	sort.size.convert = sort.size_converter(".");
	
	// Size Sort - comma decimal separator
	// ---------
	sort.size_comma = function(a,b) {
		return sort.size_comma.convert(a)-sort.size_comma.convert(b);
	};
	sort.size_comma.convert = sort.size_converter(",");
			...
			for (var i=0,L=filters.length; i<L; i++) {
				...
				if (typeof(filter.filter)=="string") {
					...
					if (filter.filter.match(/^\/(.*)\/$/)) {
						filter.filter = new RegExp(RegExp.$1, "gi");
